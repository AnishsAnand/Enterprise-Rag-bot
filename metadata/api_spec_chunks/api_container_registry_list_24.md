# API Specification: container_registry - list

**Resource:** container_registry
**Operation:** list
**Aliases:** container registry, registry, docker registry, image registry

## Endpoint
- **Method:** POST
- **URL:** {BASE_URL_PAAS_SERVICE}/api/v1/paas/listManagedServices/IKSContainerRegistry
- **Auth:** Bearer token (from Keycloak)
- **Description:** List Container Registry managed services across endpoints

## Required Parameters
- `endpoints`

## Optional Parameters
None

## Permissions
Roles: admin, developer, viewer

## Workflow Steps
### Workflow: list_container_registry
List Container Registry services across endpoints
- Step 1: get_paas_engagement (engagement.get)
- Step 2: convert_to_ipc_engagement (k8s_cluster.get_ipc_engagement) (depends on: engagement_id)
- Step 3: get_endpoints (endpoint.list) (depends on: paas_engagement_id)
- Step 4: list_container_registry_services (.)
