...
class K8sClusterAgent(BaseResourceAgent):
    ...
    def _build_formatting_prompt(
        ...
        """
        Build K8s-specific formatting prompt.
        
        Overrides base class to provide cluster-specific formatting instructions.
        """
        if operation == "list":
            return self._build_list_formatting_prompt(raw_data, user_query, context)
        elif operation == "create":
            return self._build_create_formatting_prompt(raw_data, user_query, context)
        else:
    ...
    def _build_list_formatting_prompt(
        ...
        """Build prompt for listing clusters."""
        import json
        
        return f"""You are a Kubernetes infrastructure assistant. Format the cluster listing for the user.
...
**Instructions:**
1. Start with a summary: "Found X Kubernetes cluster(s)" with location info
2. Present clusters in a clean table format with these columns:
   - Cluster Name
   - Status (use ✅ for Active/Running, ⚠️ for Pending/Provisioning, ❌ for Failed)
   - K8s Version
   - Location/Datacenter
   - Control Plane Type
...
3. Highlight important information:
   - **Bold** cluster names
   - Show status with appropriate emojis
   - Indicate production vs non-production environments
4. Group by location if multiple locations
5. Add helpful insights:
   - Mention if any clusters need updates
   - Highlight version discrepancies
   - Note any failed or pending clusters
6. End with helpful next steps like "To view details of a specific cluster, ask me about it by name"
...
| Cluster Name | Status | K8s Version | Control Plane | Nodes |
|--------------|--------|-------------|---------------|-------|
| **prod-cluster-01** | ✅ Active | 1.28.0 | managed | 5 |
| **dev-cluster-02** | ⚠️ Pending | 1.27.0 | managed | 3 |

💡 **Insights:**
- All clusters are running stable versions
- 1 cluster is currently provisioning

**Next Steps:** Ask me about any cluster by name for detailed information.
```

Do NOT include raw JSON. Provide a user-friendly, formatted response."""
    
    def _build_create_formatting_prompt(
        self,
        cluster_data: Dict[str, Any],
        user_query: str,
        context: Dict[str, Any]
    ) -> str:
        """Build prompt for cluster creation result."""
        import json
        
        return f"""You are a Kubernetes infrastructure assistant. Format the cluster creation result.
...
**Instructions:**
1. Start with: "✅ Successfully initiated cluster creation!"
2. Show key details:
   - **Cluster Name**
   - **Location**
   - **Kubernetes Version**
   - **Control Plane Type**
   - **Initial Status** (usually "Provisioning")
3. Provide timeline expectations:
   - "Your cluster is being provisioned. This typically takes 10-15 minutes."
...
Do NOT include raw JSON. Make it conversational and helpful."""
