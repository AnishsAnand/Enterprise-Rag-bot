...
class ActionBot:
    def __init__(
        ...
        """
        ai_service:
            Object providing optional helpers like parse_required_fields(...)
        internal_registry_base:
            Optional base URL for service registry
            (e.g., "http://internal-registry:8000")
        """
    ...
    async def _probe_service(self, base_url: str) -> Dict[str, Any]:
        ...
        result: Dict[str, Any] = {}
        for endpoint, key in (("health", "health"), ("versions", "versions")):
            try:
                resp = await self.client.get(f"{base_url.rstrip('/')}/{endpoint}")
                result[f"{key}_status"] = resp.status_code
                try:
                    result[f"{key}_body"] = resp.json()
                except Exception:
                    result[f"{key}_body"] = (resp.text or "")[:400]
    ...
    async def _call_service(
        ...
        try:
            resp = await self.client.request(
                method.lower(), url, headers=headers, params=params, json=json_payload, data=data
            )
            body = None
            try:
                body = resp.json()
    ...
    async def handle_action_request(self, payload: Dict[str, Any]) -> ActionResult:
        """
        Main entrypoint for executing an action request.
        payload example:
        {
        ...
        if target.startswith(("http://", "https://")):
            raw_base = target
        elif self.internal_registry_base:
        ...
        if not version:
            ...
            if not discovered_version:
                logger.info("version not found for %s; requesting input", target)
        ...
        # Interactive validation via AI service
        if payload.get("interactive", True) and hasattr(
            self.ai_service, "parse_required_fields"
        ):
            try:
                missing = await self.ai_service.parse_required_fields(
                ...
                logger.debug("parse_required_fields failed: %s", exc)

        # Attempt the HTTP call
        status_code, body = await self._call_service(
            method,
            final_url,
            headers=headers,
            params=params,
        ...
        if 200 <= status_code < 300:
            return ActionResult(
                ...
                details={"status_code": status_code, "result": body},
            )
        if status_code == 0:
