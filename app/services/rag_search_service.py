...
class EnhancedRAGSearchService:
    ...
    def __init__(self):
        ...
        self.min_relevance_threshold = float(
            getattr(settings, "MIN_RELEVANCE_THRESHOLD", 0.45) 
        )
    ...
    def _analyze_query_complexity(self, query: str) -> Dict:
        """
         Deep query analysis for adaptive thresholding
    ...
    def _calculate_adaptive_threshold(self, query_complexity: Dict) -> float:
        """
        Calculate adaptive threshold with tighter bounds
        Strategy:
        ...
        """
        base = self.min_relevance_threshold  
        ...
        logger.info(f"[Adaptive Threshold] {threshold:.2f} "f"(specificity: {specificity:.2f}, "f"type: {query_complexity['question_type']})")
        return threshold
    
    # ========================================================================
    # MULTI-STAGE SEMANTIC VALIDATION
    # ========================================================================
    def _semantic_filter_results(self,query: str,results: List[Dict],min_semantic_score: float = 0.40) -> List[Dict]:
        """
        Pre-filter results using semantic keyword matching
        This eliminates clearly irrelevant results before expensive re-ranking.
        Semantic matching:
    ...
    def _cross_reference_validate(self,query: str,chunks: List[Dict]) -> List[Dict]:
        """
        Cross-reference validation with peer support analysis
        Validates chunks by:
    ...
    def _calculate_enhanced_confidence(self,query: str,chunks: List[Dict],query_complexity: Dict) -> float:
        """
        ...
        if len(chunks) > 1:
            ...
            consistency = max(0, 1.0 - (score_variance * 2)) 
        else:
            consistency = 1.0
        ...
        validation_scores = [c.get("validation_metadata", {}).get("support_strength", 0)for c in chunks]
        avg_validation = np.mean(validation_scores) if validation_scores else 0
        validation_contribution = avg_validation * 0.10
        # Combine factors
        confidence = (relevance_contribution +top_contribution +consistency_contribution +alignment_contribution +validation_contribution)
    ...
    def _deduplicate_semantically(self, chunks: List[Dict]) -> List[Dict]:
        """
        Remove semantically similar (near-duplicate) chunks
        Uses:
    ...
    # ========================================================================
    async def search(self,query: str,top_k: int = 10,user_id: Optional[int] = None) -> Dict:
        """
        RAG search 
        Flow:
        ...
        try:
            ...
            semantic_filtered = self._semantic_filter_results(query,initial_results,min_semantic_score=0.35 )
            ...
            strict_filtered = [c for c in deduplicated if c.get("relevance_score", 0) >= adaptive_threshold]
            if len(strict_filtered) < 3:
                relaxed_threshold = max(0.30, adaptive_threshold - 0.15)
                final_results = [
                    c for c in deduplicated
                    if c.get("relevance_score", 0) >= relaxed_threshold][:top_k]
                logger.info(
            ...
            confidence = self._calculate_enhanced_confidence(query,final_results,query_complexity)
