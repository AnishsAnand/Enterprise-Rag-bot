# user_main.py (updated - ready to paste)
import os
import logging
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.exception("ChromaDB initialization failed (continuing): %s", e)
...
async def user_chat_query(request: UserQueryRequest):
    ...
    Enhanced fallback will perform broader chroma search + generate stepwise response
    ...
    try:
        ...
        widget_max = max(request.max_results, 50)
        ...
        try:
            # broaden chroma results
            chroma_n = min(200, max(request.max_results * 3, 120))
            chroma_results = []
            try:
                chroma_results = await chroma_service.search_documents(query, n_results=chroma_n)
            except Exception as e:
                logger.exception("Direct chroma_service.search_documents failed: %s", e)
                chroma_results = []

            if chroma_results:
                # build contexts from top docs
                context_texts = []
                for r in chroma_results[:min(40, len(chroma_results))]:
                    content = r.get("content", "") or ""
                    if content and len(content.strip()) > 50:
                        context_texts.append(content[:5000])
                if not context_texts:
                    context_texts = [r.get("content", "")[:2000] for r in chroma_results[:3]]
                ...
                # Collect candidate images from chroma metadata
                candidate_images = []
                seen_urls = set()
                ...
                final_resp = {
                    ...
                    "results_found": len(chroma_results),
                    "results_used": min(len(chroma_results), request.max_results),
                ...
                logger.warning("Chroma returned no additional documents during enhanced retrieval.")
