...
async def update_credentials(request: UpdateCredentialsRequest,current_user: AuthUser = Depends(get_current_user)):
    """
    ...
    try:
        ...
        if success:
            ...
            return CredentialsResponse(
                success=True,message="API credentials updated successfully",has_credentials=True)
        else:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,detail="Failed to update credentials"
            )
...
async def get_credentials_status(current_user: AuthUser = Depends(get_current_user)):
    """
    ...
    try:
        ...
        return CredentialsResponse(success=True,message="Credentials status retrieved",has_credentials=has_credentials)
    except Exception as e:
        logger.error(f"❌ Error checking credentials: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,detail=f"Error checking credentials: {str(e)}")
