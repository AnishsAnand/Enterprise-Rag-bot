...
async def login_with_tata_validation(request: LoginRequest, response: Response):
    ...
    try:
        ...
        stored = user_credentials_service.store_credentials(
            email=request.email,
        ...
        validation_result = await tata_auth_service.validate_user(
            email=request.email,
            password=request.password
        )
        ...
        return LoginResponse(
            ...
            token=token,
            access_level=access_level,
            user_info=validation_result.get("user_info", {"email": request.email}),
            message=message
        )
...
async def setup_credentials(request: SetupCredentialsRequest):
    ...
    This endpoint allows users to set up their Tata API credentials after signing up in OpenWebUI.
    
    Flow:
    1. User signs up in OpenWebUI
