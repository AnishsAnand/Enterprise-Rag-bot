...
Provides WebUI-style endpoints for chat persistence
Routes:
...
async def get_chat_list(request: Request,page: Optional[int] = None,include_pinned: Optional[bool] = False,include_folders: Optional[bool] = False,db: Session = Depends(get_db)):
    """
...
async def search_chats(request: Request,text: str,page: Optional[int] = 1,db: Session = Depends(get_db)):
...
async def get_pinned_chats(request: Request,db: Session = Depends(get_db)):
...
async def get_archived_chats(request: Request,page: Optional[int] = 1,db: Session = Depends(get_db)):
...
async def create_new_chat(request: Request,form_data: ChatForm,db: Session = Depends(get_db)):
    """Create a new chat"""
    user_id = get_current_user_id(request)
    try:
...
async def get_chat_by_id(chat_id: str,request: Request,db: Session = Depends(get_db)):
...
async def update_chat_by_id(chat_id: str,form_data: ChatForm,request: Request,db: Session = Depends(get_db)):
    """Update a chat by ID"""
    user_id = get_current_user_id(request)
    # Verify ownership
...
async def delete_chat_by_id(chat_id: str,request: Request,db: Session = Depends(get_db)):
    """Delete a chat by ID"""
    user_id = get_current_user_id(request)
...
async def toggle_pin_chat(chat_id: str,request: Request,db: Session = Depends(get_db)):
    ...
    if chat:
        return ChatResponse(**chat.model_dump())
    raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,detail="Failed to toggle pin")

@router.post("/{chat_id}/archive", response_model=Optional[ChatResponse])
async def toggle_archive_chat(chat_id: str,request: Request,db: Session = Depends(get_db)):
    ...
    if chat:
        return ChatResponse(**chat.model_dump())
    raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,detail="Failed to toggle archive")
...
async def update_chat_message(chat_id: str,message_id: str,form_data: MessageForm,request: Request,db: Session = Depends(get_db)):
    ...
    if not existing:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="Chat not found" )
    chat = chat_service.upsert_message_to_chat(chat_id,message_id,{"content": form_data.content},db)
    if chat:
        return ChatResponse(**chat.model_dump())
...
async def get_chat_tags(chat_id: str,request: Request,db: Session = Depends(get_db)):
...
async def add_chat_tag(chat_id: str,form_data: TagForm,request: Request,db: Session = Depends(get_db)):
    """Add a tag to a chat"""
    user_id = get_current_user_id(request)
    if form_data.name.lower() == "none":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,detail="Tag name cannot be 'None'" )
...
async def remove_chat_tag(chat_id: str,form_data: TagForm,request: Request,db: Session = Depends(get_db)):
...
async def update_chat_folder(chat_id: str,form_data: FolderForm,request: Request,db: Session = Depends(get_db)):
