...
async def call_maybe_async(fn, *args, **kwargs):
    ...
    This allows service objects (chroma_service, ai_service) to expose sync or async methods.
    """
    if not callable(fn):
...
async def widget_query(request: WidgetQueryRequest):
    ...
    try:
        ...
        # If step-level images were returned by the step generator, prefer/merge them
        # (this keeps compatibility with older flows that returned step images)
        step_level_images_added = 0
        for step in steps_data:
            if isinstance(step, dict):
                # Common keys we might see from an image-aware step generator
                possible_img = step.get("image") or step.get("image_url") or step.get("image_prompt") or step.get("image_data")
                ...
                elif isinstance(possible_img, str) and possible_img.startswith("http"):
                    url = possible_img
                    if url not in seen_urls:
                        img_obj = {
                            "url": url,
        ...
        if step_level_images_added:
            ...
            logger.debug(f"Added {step_level_images_added} step-level images to selected_images.")
        ...
        for i, step in enumerate(steps_data):
            ...
            # Prefer an explicit image assigned to the step if present, otherwise use from selected_images by order
            assigned_img = None
            if isinstance(step, dict) and step.get("image"):
                ...
                if isinstance(si, dict) and si.get("url"):
                    assigned_img = {
                        "url": si.get("url"),
                        "alt": si.get("alt", "") or "",
                        "caption": si.get("caption", "") or "",
                        "relevance_score": si.get("relevance_score", None),
                    }
                elif isinstance(si, str) and si.startswith("http"):
                    assigned_img = {"url": si, "alt": "", "caption": "", "relevance_score": None}

            # If no explicit image for this step, use the Nth selected image if available
...
async def widget_upload_file(file: UploadFile = File(...), store_in_knowledge: bool = True, chunk_large_files: bool = True):
    """
    Enhanced file upload with robust processing, metadata extraction, chunking and
    resilient storage into the knowledge base (tries multiple chroma_service methods).
    """
    try:
        ...
        async def _store_documents_safe(docs: List[Dict[str, Any]]):
            ...
            for name in candidates:
                if hasattr(chroma_service, name):
                    func = getattr(chroma_service, name)
                    break
            if not func:
                raise RuntimeError("chroma_service has no storage method (expected add_documents/store_documents)")
...
async def widget_knowledge_stats():
    ...
    try:
        stats = await call_maybe_async(getattr(chroma_service, "get_collection_stats", chroma_service))
...
async def widget_clear_knowledge():
    ...
    try:
        ...
        await call_maybe_async(getattr(chroma_service, "delete_collection", chroma_service))
        await call_maybe_async(getattr(chroma_service, "initialize", chroma_service))
        return {"status": "success", "message": "Knowledge base cleared and reinitialized successfully", "timestamp": datetime.now().isoformat()}
    except Exception as e:
...
async def store_document_task(docs: List[Dict[str, Any]]):
    """Background storage task (safe for sync/async chroma_service.store_documents)"""
    try:
        func = getattr(chroma_service, "store_documents", None) or getattr(chroma_service, "add_documents", None)
        if not func:
            raise RuntimeError("chroma_service lacks a store_documents/add_documents function")
        res = func(docs)
        if inspect.isawaitable(res):
            await res
...
async def enhanced_bulk_scrape_task(urls: List[str], auto_store: bool, max_depth: int):
    """Enhanced bulk scraping with better progress tracking and error handling"""
    scraped_count = 0
    stored_count = 0
    error_count = 0
    ...
    for i in range(0, len(urls), batch_size):
        ...
        for url, result in zip(batch, results):
            ...
            if result.get("status") == "success" and result.get("content"):
                ...
                if page_text and len(page_text.strip()) >= 100:
                    ...
                    logger.warning(f"Skipping {url}: content too short ({len(page_text)} chars)")
            else:
                error_count += 1
        ...
        if documents_to_store:
            try:
                # prefer add_documents, then store_documents (or fallback)
                store_fn = getattr(chroma_service, "add_documents", None) or getattr(chroma_service, "store_documents", None)
                if not store_fn:
                    raise RuntimeError("No storage function found on chroma_service for bulk store")
        ...
        logger.info(f"Batch {i//batch_size + 1}/{(len(urls)-1)//batch_size + 1} completed in {batch_duration:.1f}s")

        if batch_errors:
            logger.warning(f"Batch errors: {', '.join(batch_errors[:3])}")
